...
initDb();
const adminBootstrap = bootstrapAdminUser();
...
app.post('/api/auth/login', (req, res) => {
  ...
  try {
    const session = loginAndIssueSession(email, password);
...
app.post('/api/auth/register', (req, res) => {
  const email = String(req.body?.email || '').trim().toLowerCase();
  const password = String(req.body?.password || '');
  if (!getRegistrationEnabled()) return res.status(403).json({ error: 'Account registration is currently disabled' });
  try {
    const session = registerAndLogin(email, password);
    return res.json(session);
  } catch (error) {
...
app.post('/api/auth/change-password', requireAuth, (req, res) => {
  ...
  try {
    const user = changePassword(req.auth, currentPassword, newPassword);
    return res.json({ user });
  } catch (error) {
    return res.status(400).json({ error: String(error.message || error) });
  }
...
app.get('/api/user/settings', requireAuth, (req, res) => {
  try {
    const settings = getUserAiSettings(req.auth.email);
...
app.put('/api/user/settings', requireAuth, (req, res) => {
  ...
  try {
    const saved = saveUserAiSettings(req.auth.email, settings, { keepExistingKey });
...
app.put('/api/admin/settings/registration', requireAuth, requireAdmin, (req, res) => {
  try {
    ...
    const registrationEnabled = setRegistrationEnabled(enabled);
...
app.put('/api/admin/users/:email/lock', requireAuth, requireAdmin, (req, res) => {
  try {
    const email = decodeURIComponent(req.params.email || '').trim().toLowerCase();
    const locked = Boolean(req.body?.locked);
    setUserLockedByAdmin(req.auth.email, email, locked);
...
app.put('/api/admin/users/:email/role', requireAuth, requireAdmin, (req, res) => {
  try {
    const email = decodeURIComponent(req.params.email || '').trim().toLowerCase();
    const role = String(req.body?.role || 'user');
    setUserRoleByAdmin(req.auth.email, email, role);
    return res.json({ ok: true });
  } catch (error) {
    return res.status(400).json({ error: String(error.message || error) });
  }
...
app.delete('/api/admin/users/:email', requireAuth, requireAdmin, (req, res) => {
  try {
    const email = decodeURIComponent(req.params.email || '').trim().toLowerCase();
    deleteUserByAdmin(req.auth.email, email);
    return res.json({ ok: true });
  } catch (error) {
    return res.status(400).json({ error: String(error.message || error) });
  }
...
app.get('/api/stories', requireAuth, requirePasswordFresh, (req, res) => {
  try {
    return res.json({ stories: listStoriesByUser(req.auth.email) });
  } catch {
    return res.status(500).json({ error: 'Failed to fetch stories' });
  }
});

app.post('/api/stories', requireAuth, requirePasswordFresh, (req, res) => {
  const story = req.body?.story;
  if (!story?.id) return res.status(400).json({ error: 'story payload is required' });
  try {
    return res.status(201).json({ story: createStory(req.auth.email, story) });
  } catch (error) {
    return res.status(400).json({ error: String(error.message || error) });
  }
});

app.put('/api/stories/:storyId', requireAuth, requirePasswordFresh, (req, res) => {
  const story = req.body?.story;
  const storyId = req.params.storyId;
  if (!story || story.id !== storyId) return res.status(400).json({ error: 'matching story is required' });

  try {
    return res.json({ story: saveStory(req.auth.email, story) });
  } catch (error) {
    return res.status(400).json({ error: String(error.message || error) });
  }
});

app.post('/api/stories/sync', requireAuth, requirePasswordFresh, (req, res) => {
  const stories = req.body?.stories;
  if (!Array.isArray(stories)) return res.status(400).json({ error: 'stories[] are required' });

  try {
    return res.json(syncStories(req.auth.email, stories));
  } catch (error) {
    return res.status(400).json({ error: String(error.message || error) });
  }
});

app.delete('/api/stories/:storyId', requireAuth, requirePasswordFresh, (req, res) => {
  try {
    return res.json({ deleted: deleteStory(req.auth.email, req.params.storyId) });
  } catch {
...
async function handleAi(req, res, task, fn) {
  ...
  try {
    recordAiRun({ storyId, actorEmail, task, status: 'started' });
    const data = await fn();
    recordAiRun({ storyId, actorEmail, task, status: 'succeeded' });
    return res.json(data);
  } catch (error) {
    recordAiRun({ storyId, actorEmail, task, status: 'failed', errorMessage: String(error) });
